...
Public Sub SyncronizeWithSQLServer(Optional ByVal SyncEvenIfLocal As Boolean = False)
    ...
    If Not SyncEvenIfLocal Then
        If CSM_Session.GetComputerName() = pDatabase.DataSource Then
            WriteLogEvent "Synchronizing Date/Time with SQL Server - SKIPPED because this machine is the server.", vbLogEventTypeInformation
            Exit Sub
        End If
    End If
    ...
    WriteLogEvent "Synchronizing Date/Time with SQL Server - OLD: " & Format(Date, "Short Date") & " " & Format(Time, "Short Time") & " - NEW: " & Format(CDate(recData("Fecha").Value), "Short Date") & " " & Format(CDate(recData("Hora").Value), "Short Time"), vbLogEventTypeInformation
